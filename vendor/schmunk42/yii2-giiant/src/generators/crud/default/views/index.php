...
Yii::$app->view->params['pageButtons'] = Html::a('<span
    class="fa fa-plus"></span> ' . <?= $generator->generateString('New') ?>, ['create'], ['class' => 'btn btn-success']);
...
<div class="giiant-crud <?= Inflector::camel2id(StringHelper::basename($generator->modelClass), '-', true) ?>-index">
    ...
    if ($generator->accessFilter) {
        ...
        ?>
        <div class="pull-left">
            <?= '<?= ' ?>Html::a('<span class="fa fa-plus"></span> ' . <?= $generator->generateString(
                'New'
    ...
    <div class="pull-right">
        ...
        <?php foreach ($generator->getModelRelations($model) as $relation): ?>
            ...
            [
                'url' => ['{$route}'],
                'label' => '<i class="fa fa-arrow-right">&nbsp;' . {$generator->generateString($label)} . '</i>',
            ],
PHP;
            ?>
        <?php endforeach; ?>
    ...
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">Data <?= "<?= Yii::t('{$generator->messageCategory}', '{$modelName}') ?>" ?></h3>
        </div>
